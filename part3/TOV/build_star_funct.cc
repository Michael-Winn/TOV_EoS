...
void calculation_for_list_of_EOS(struct constant_variables variables_set, int Number_of_files){
  ...
  for (int i = 0; i< Number_of_files; i++){
  //string input_file = "input/EOS/eos_" + to_string(i) + "_.out";
  string input_file = "../EOS_maker/NSEoS/source/apps/stats/EOS/eos_" + to_string(i) + "_.out";
  EOS_DATA_FILE.open(input_file);
        if(EOS_DATA_FILE.fail())
    {
        fprintf(stderr, "ERROR: file issue in calculation list EOS\n");
        exit(1);
      }
